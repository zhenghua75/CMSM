	...
	public class frmDataManuUpdate_His : frmBase
	{
		...
		private void UpdateData()
		{
			...
			catch(Exception conerr)
			{
				...
				hsMessage.Add(hsMessage.Count,"连接中心数据库失败，请检查网络！");
...
//					{
//						hsMessage.Add(hsMessage.Count,"--------------------------------");
//						hsMessage.Add(hsMessage.Count,DateTime.Now.ToLongTimeString()+"  开始同步员工资料...");
//						#region 加载员工资料
//						//本地员工资料
...
//						#endregion
//						hsMessage.Add(hsMessage.Count,"更新中心数据：员工资料修改。共修改"+UpdatedCount.ToString()+"条记录.");
//						UpdatedCount=0;
//						#region 本地向中心同步员工资料--插入
//						sd = new SqlDataAdapter();
...
//						#endregion
//						hsMessage.Add(hsMessage.Count,"更新中心数据：员工资料新增。共新增"+UpdatedCount.ToString()+"条记录.");
//						hsMessage.Add(hsMessage.Count,"--------------------------------");
//						hsMessage.Add(hsMessage.Count,DateTime.Now.ToLongTimeString()+"  开始同步员工资料至本地...");
//						UpdatedCount=0;
//						#region 中心向本地同步员工资料--插入
//						//删除本地员工资料
...
//						dsEmpInsertSync.Dispose();
//						#endregion
//						hsMessage.Add(hsMessage.Count,"更新本地数据：员工资料重新下载。共下载"+UpdatedCount.ToString()+"条记录.");
//
//						tranLocal.Commit();
//						tranCenter.Commit();
//
//						hsMessage.Add(hsMessage.Count,DateTime.Now.ToLongTimeString()+"同步员工资料成功!");
//						hsMessage.Add(hsMessage.Count,"--------------------------------");
//					}
//					catch(Exception ex)
//					{
...
//			conCenter.Close();
			...
			try
			{
				...
				//充值撤消
				sql="select iSerial from tbFillFeeHis where vcDeptID='"+SysInitial.LocalDept+"' and dtFillDate>='" + strBeginDate + "' and vcComments='充值撤消'";
				cmd1=new SqlCommand(sql,conLocal);
				cmd1.CommandTimeout=600;
				daTmp1=new SqlDataAdapter(cmd1);
				...
				#endregion
				hsMessage.Add(hsMessage.Count,"加载本地数据库资料成功！");
				hsMessage.Add(hsMessage.Count,"--------------------------------");
				...
				using(SqlTransaction tranCenter=conCenter.BeginTransaction(IsolationLevel.ReadCommitted))
				{
					...
					hsMessage.Add(hsMessage.Count,"--------------------------------");
					hsMessage.Add(hsMessage.Count,DateTime.Now.ToLongTimeString()+"  开始更新中心数据：考勤信息...");

					UpdatedCount=0;
					#region 向中心同步本店考勤_以注销
//					sd = new SqlDataAdapter();
...
//					dsEmpSignSync.Dispose();
					#endregion
//					hsMessage.Add(hsMessage.Count,"更新中心数据：考勤信息更新成功。共更新"+UpdatedCount.ToString()+"条记录.");
//					hsMessage.Add(hsMessage.Count,"--------------------------------");
//					hsMessage.Add(hsMessage.Count,DateTime.Now.ToLongTimeString()+"  开始更新中心数据：员工资料信息...");
...
//					conLocal.Close();
					...
					if(strFillRemoveSerial!="")
					{
						sql="update tbFillFeeOther set vcComments='充值撤消' where vcComments<>'充值撤消' and vcDeptID='"+SysInitial.LocalDept+"' and iSerial in("+strFillRemoveSerial+")";
						cmd=new SqlCommand(sql,conCenter,tranCenter);
						cmd.CommandTimeout=600;
						cmd.ExecuteNonQuery();
					}
