...
namespace AMSSync
{
	...
	public class ConfigAdapter
	{
        ...
        public static string GetConfigNote(string strNode,string strNoteName,string strKey="key",string strValue="value")
        {
            ...
            foreach(XmlNode ne in _node)
            {
                if (ne.NodeType == XmlNodeType.Element )
                { //只对Element操作
                    if (ne.Attributes[strKey].Value.ToLower() == strNoteName.ToLower())
                    {
                        strNodeValue = ne.Attributes[strValue].Value; // 取节点值
                        break;
                    }
                }
            }
